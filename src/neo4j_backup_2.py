...
def main():
    ...
    print("go children", elapsed.total_seconds(), last_round.total_seconds())
    ...
    print("connect_go_parents", elapsed.total_seconds(), last_round.total_seconds())
    ...
    send_to_neo4j(driver, 'CREATE INDEX ON :EditableMCodeList(id)')
    read_editable_mcode_list = '''LOAD CSV WITH HEADERS FROM 'file:///EditableMCodeList.csv' AS row
                                   WITH row.field as field, row.edit_date as editDate, row.editor_id as editor, row.EditableDiseaseList_graph_id as id
                                   MATCH(u:User) WHERE u.id=editor
                                   CREATE (emcdl:EditableMCodeList :EditableObject {field:field, editDate:editDate,  id:id}) 
                                   CREATE (emcdl)-[:EDITED_BY]->(u)'''
    send_to_neo4j(driver, read_editable_mcode_list)
    elapsed, last_round, now = get_elapsed_time(now, start)
    print("EditableMCodeList", elapsed.total_seconds(), last_round.total_seconds())

    read_editable_mcode_list_elements = '''LOAD CSV WITH HEADERS FROM 'file:///EditableMCodeListElements.csv' AS row
                                      WITH row.EditableDiseaseList_graph_id as editable, split(row.Disease_graph_id, "|") AS Disease_graph_id
                                      MATCH(emcdl:EditableMCodeList) WHERE emcdl.id=editable 
                                      SET emcdl.list = Disease_graph_id '''
    send_to_neo4j(driver, read_editable_mcode_list_elements)
    print("EditableMCodeListElements", elapsed.total_seconds(), last_round.total_seconds())

    read_mcode_list_to_oncotree_disease = ''' MATCH (emcdl:EditableMCodeList) UNWIND emcdl.list as mc_diseases  MATCH (mc:MCode  {id:mc_diseases})  
                              CREATE (emcdl)-[:MCODE]->(mc)  '''
    send_to_neo4j(driver, read_mcode_list_to_oncotree_disease)
    print("Match editable mcodes", elapsed.total_seconds(), last_round.total_seconds())
    ...
    read_ontological_diseases = '''LOAD CSV WITH HEADERS FROM 'file:///ontological_diseases.csv' AS row
                                          ...
                                          row.goDiseases as goDiseases, row.oncoTreeDiseases as oncoTreeDiseases, row.xrefs as xrefs, row. MCodes as mCodes,  
                                          ...
                                          MATCH(xreflist:EditableXRefList) WHERE xreflist.id=xrefs
                                          MATCH(om:EditableMCodeList) WHERE om.id=mCodes
